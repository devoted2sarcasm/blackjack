...
void dealerTurn(vector<Card>& deck, vector<vector<vector<Card>>>& hands) {
    ...
    if (handValue(hands.back()[0]) >= 17 && !isBust(hands.back()[0])) {
        cout << "Dealer stays with " << handValue(hands.back()[0]) << endl << endl;
        return;
    }

    if (isBust(hands.back()[0])) {
        cout << "Dealer has busted!" << endl << endl;
        return;
    }
    cout << endl;
    return;
    ...
    for (int i = 0; i < hands.size() - 1; i++) {
        for (int j = 0; j < hands[i].size(); j++) {
            ...
            if (isBlackjack(hands[i][j])) {
                if (isBlackjack(hands.back()[0])) {
                    cout << "Player " << i + 1 << " pushes with blackjack." << endl;
                ...
                if (hands[i][j].back().doubled == true) {
                    playerMoney[i] += bets[i] * 4;
                }
                else {
...
void hitStand(vector<Card>& hand, vector<Card>& deck, int& playerMoney, int bet) {
    ...
    if (hit == 'd') {
        ...
        //hits card, ends hitstand function, takes additional money from palyer
            ...
            cout << "Doubling down, bet for this hand is now: $" << bet*2 << endl;
            playerMoney -= bet;
            deck.back().doubled = true;
...
//offer of insurance and bypass other steps
bool insurance(vector<vector<vector<Card>>> hands, vector<int>& playerMoney, vector<int> bets) {
    ...
    for (int i = 0; i < bets.size(); i++) {
        ...
        while (ins != 'y' && ins != 'n') {
            cin.clear();
            cout << "Please enter 'y' or 'n' for insurance." << endl;
            cin >> ins;
        }
        if (ins == 'y') {
            ...
            int ins_amt;
            cin >> ins_amt;
            while (ins_amt < 1 || ins_amt > (bets[i] / 2)) {
                cin.clear();
                cout << "Please enter a valid amount: (1-" << bets[i] / 2 << ")" << endl;
                cin >> ins_amt;
            }*/
            insurance[i] = (bets[i] / 2);
        }
        else {
            cout << "Player " << i + 1 << "has declined insurance." << endl;
        }
    }
    cout << "All players have optioned." << endl;
    if (isBlackjack(hands.back()[0])) {
        hands.back()[0][1].hidden = false;
        printDealerHand(hands.back()[0]);
        cout << "Dealer has blackjack! Insurance was a good choice." << endl;
        for (int i = 0; i < playerMoney.size(); i++) {
            playerMoney[i] += (insurance[i] * 2);
            cout << "Player " << i + 1 << " paid " << (insurance[i] * 2) << "." << endl;
            return true;
        }
    }
    else {
